const Concelho = require("../models/concelho.js");
...
{
    let concelho = new Concelho(obj);

    concelho.save(function (error)
    {
        if (error) return console.error(error);
        console.log("INSERT Concelho:", obj);
    });
...
function update(data, callback)
{
    ...
    Concelho.findOneAndUpdate(filter, { $set: dataToUpdate }, function(error, result)
    {
...
{
    Concelho.findOneAndDelete({_id: id }, function(error, result)
    {
        if (error) console.log(error);
        else callback(result);
    });
...
{
    Concelho.find({}).exec(function(error, results)
    {
        if (error) return console.error(error);
        callback(results);
    });
...
function find(column, value, one, callback)
{
    ...
    if (one)
    {
        Concelho.findOne(search).exec(function(error, result)
        {
            if (error) return console.error(error);
            callback(result);
        });
    ...
    {
        Concelho.find(search).exec(function(error, results)
        {
            if (error) return console.error(error);
            callback(results);
        });
